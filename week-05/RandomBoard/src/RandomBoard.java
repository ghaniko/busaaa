...
public class RandomBoard {
  public static void main(String[] args) {
    ...
    while (actualPositionOfTiles.size() < numberOfTiles){
      ...
      if (forbiddenPositionOfTiles.contains(actualPositionOfTiles.get(randomIndex) + 1) &&
        ...
        if (randomDirection == 0) {
          checkOrFill(forbiddenPositionOfTiles, actualPositionOfTiles, randomIndex, -(width + 2), randomDirection);
        } else if (randomDirection == 1) {
          checkOrFill(forbiddenPositionOfTiles, actualPositionOfTiles, randomIndex, 1, randomDirection);
    ...
    //Transform it to an up-loadable format
    decreaseAllElement(actualPositionOfTiles, width);
    Collections.sort(actualPositionOfTiles);
    transformToArray(transformToString(transformToCharArray(actualPositionOfTiles, width, height)), width, height);
  }

  public static void checkOrFill(ArrayList<Integer> forbiddenPositionOfTiles, ArrayList<Integer> actualPositionOfTiles,
                                   int randomIndex, int neighbourPosition, int randomDirection){
  ...
  /*public static ArrayList<Integer> chopOffSides(ArrayList<Integer> inputArrayList, int width, int height){
    for (int i = 1; i <= width + 2; i++) {
      inputArrayList.remove(inputArrayList.indexOf(i));
      inputArrayList.remove(inputArrayList.indexOf((height + 2) * (width + 2) + 1 - i));
      if(i % (width + 2) == 1 || i % (width + 2) == 0){
        for (int j = 1; j <= height; j++) {
          inputArrayList.remove(inputArrayList.indexOf(i + j * (width + 2)));
        }
      }
    }
    return inputArrayList;
  ...
  public static char[] transformToCharArray(ArrayList<Integer> arrayListToBeTransformed, int width, int height) {
    ...
    //String stringToBeSplit = Arrays.toString(helpCharArray);
